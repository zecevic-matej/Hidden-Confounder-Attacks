...
class HouseModel_Gas:
    """Create an instance of the House model, setting its basic parameters"""

    def __init__(self, *settings_dict):
        if not settings_dict:
            self.Settings = getSettings_Gas()
        else:
            self.Settings = settings_dict[0]

    def sample_model(self, input_scales=None, input_changes=None, fixed_outputs=None):
        ...
        # Input Scales
        Names = ["CostPV", "CostBat", "CostBuy", "Demand", "CostGas"]
        Scaling = [1, 1, 1, 1, 1, 1, 1, 1, 1]
        if input_scales is not None:
            for i in range(Names.__len__()):
                try:
                    Scaling[i] = input_scales[Names[i]]
                except:
                    Scaling[i] = 1
        ...
        if input_changes is not None:
            for key in self.Settings.keys():
                if key in input_changes.keys():
                    self.Settings[key] = input_changes[key]
        ...
        cost_Gas = self.Settings["cost_gas"]
        ...
        with open('./energy_system_models/TS_PVAvail.csv', 'r') as file:
            # next(file)
            reader = csv.reader(file, delimiter='\n')
            for row in reader:
                availability_pv.append(float(row[0]))

        with open('./energy_system_models/TS_Demand.csv', 'r') as file:
            # next(file)
            reader = csv.reader(file, delimiter='\n')
            for row in reader:
        ...
        model.CostGas = pyo.Var(within=pyo.Reals)
        model.GasBuy = pyo.Var(time, within=pyo.NonNegativeReals)
        model.CapacityGas = pyo.Var(within=pyo.NonNegativeReals)

        # Step 3: Define Objective
        model.cost = pyo.Objective(expr=cost_PV * model.CapacityPV + cost_buy_ele * sum(
            model.EnergyBuy[i] for i in time) + cost_Battery * model.CapacityBattery + cost_Gas * sum(
            model.GasBuy[i] for i in time), sense=pyo.minimize)
        ...
        for i in time:
            ...
            #model.limEQ.add(model.EnergyBattery[i] <= model.CapacityGas)  # Gas Upper Limit
        ...
        for i in time:
            model.EnergyEQ.add(
                expr=model.Demand[i] == model.EnergyBuy[i] + model.EnergyBattery_OUT[i] - model.EnergyBattery_IN[
                    i] + model.EnergyPV[i] + model.GasBuy[i])  # Energy Equation
        ...
        model.ValueCostGas = pyo.Constraint(expr=model.CostGas == cost_Gas)
        ...
        solver_options = open("./energy_system_models/solverSettings.txt", "r").read().split("\n")
...
def getKPI_Gas(model, *base_model):
    from pyomo.opt import TerminationCondition as TC
    if model[1] == TC.infeasible:
        KPIdict = {"Cap_PV": -2, "Cap_Bat": -2, "Own_Gen": -2, "TOTEX": -2, "CAPEX": -2, "Gas_Bought": -2}
    else:
        ...
        cost_Gas = pyo.value(model.CostGas)
        ...
        Value_GasBought = sum(pyo.value(model.GasBuy[i]) for i in time_steps)
        Value_TotalCost = cost_PV * pyo.value(model.CapacityPV) + cost_buy_ele * sum(
            pyo.value(model.EnergyBuy[i]) for i in time_steps) + cost_Battery * pyo.value(model.CapacityBattery) + cost_Gas * sum(
            pyo.value(model.GasBuy[i]) for i in time_steps)
        Value_EleBought = sum(pyo.value(model.EnergyBuy[i]) for i in time_steps)
        ...
        if not base_model:
            KPIdict = {
                ...
                "TOTEX": Value_TOTEX,
                "CAPEX": Value_CAPEX,
                "Gas_Bought": Value_GasBought,
                "Total_Cost": Value_TotalCost,
                "Ele_Bought": Value_EleBought
            }
        else:
            ...
            KPIdict = {  # % difference to base model
                ...
                "Gas_Bought": (Value_GasBought - base_model["Gas_Bought"]) / (base_model["Gas_Bought"] + 1e-8),
                "Total_Cost": (Value_TotalCost - base_model["Total_Cost"]) / (base_model["Total_Cost"] + 1e-8),
                "Ele_Bought": (Value_EleBought - base_model["Ele_Bought"]) / (base_model["Ele_Bought"] + 1e-8)
            }
    return KPIdict